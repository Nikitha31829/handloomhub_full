...
// helper: choose a representative image for a region using products data
function getRegionImage(region) {
  const match = products.find(
    (p) =>
      p.region?.toLowerCase() === region.toLowerCase() ||
      p.category?.toLowerCase().includes(region.toLowerCase()) ||
      p.tags?.some?.(t => t.toLowerCase().includes(region.toLowerCase()))
  );
  const fallback = "https://via.placeholder.com/600x400.png?text=" + encodeURIComponent(region);
  return match?.images?.[0] || match?.image || fallback;
}
    ...
    <div className='container-px py-10 grid lg:grid-cols-3 gap-8'>
      <div className='lg:col-span-2'>
        ...
        <div className='bg-white rounded-2xl shadow-soft divide-y'>
          ...
          {items.map(i => {
            const image = getRegionImage(i.region);
            return (
              <div key={i.id} className='p-6 flex flex-col sm:flex-row items-start sm:items-center gap-4'>
                <img src={image} alt={i.title} className='h-20 w-20 rounded-lg object-cover'/>
